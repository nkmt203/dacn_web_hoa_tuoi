...
<body>
    <h2 class="mb-4 text-center">Sản phẩm có khuyến mãi tạm ngưng</h2>
    <?php
    ...
    <table class="table table-striped">
        ...
        <tbody>
            <?php foreach ($listInactive as $item): ?>
                <?php
                // Tính giá sau giảm
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if ($item['status'] === 'inactive') {
                            ...
                            echo ' <span class="badge bg-warning text-dark">Lỗi</span>';
                        }
                        ?>
                    </td>
        ...
        <nav>
            <ul class="pagination pagination-sm2">
                <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
                    <a class="page-link" href="index.php?controller=promotion&action=listInactiveProductPromotion&page=<?= $page - 1 ?>">«</a>
                </li>

                <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                    <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                        <a class="page-link" href="index.php?controller=promotion&action=listInactiveProductPromotion&page=<?= $i ?>">
                            <?= $i ?>
                        </a>
                    </li>
                <?php endfor; ?>
